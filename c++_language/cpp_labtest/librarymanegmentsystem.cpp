...
class LibraryItem // main library class 
{
private:
    string title, author, dueDate;   // common attributes
...
class Book : public LibraryItem    //  book class under library 
{
private:
    string isbn;
...
public:
    Book(string title, string author, string dueDate, string isbn) 
    {
        ...
        setAuthor(author);
        setDueDate(dueDate);
        if (!regex_match(isbn, regex("\\d{3}-\\d{10}"))) // regex to verify item  id 
        {
...
class DVD : public LibraryItem // dvd class under library 
{
private:
    int duration;   // unique attribute to class
...
class Magazine : public LibraryItem // Magazine class under library
{
private:
    int issueNumber;  // unique attributes in  class
...
LibraryItem* libraryItems[MAX_ITEMS]; // item counter
int itemCount = 0;
...
void searchByTitle(const string& searchTitle) // search by title
{
    bool found = false;
...
int main() // main block
{
    ...
    do 
    {
        ...
        try 
        {
            if (choice == 1 && itemCount < MAX_ITEMS) // add book 
            {
            ...
            else if (choice == 2 && itemCount < MAX_ITEMS) // add dvd
            {
            ...
            else if (choice == 3 && itemCount < MAX_ITEMS) // add Magazine
            {
                string title, author, dueDate;
                int issue;
                cout << "Enter Title, ";
            ...
            else if (choice == 4) // display all item
            {
                for (int i = 0; i < itemCount; i++) 
                {
                    libraryItems[i]->displayDetails();
            ...
            else if (choice == 5 || choice == 6) // item mapipulation
            {
                int idx;
            ...
            else if (choice == 7) //search by title
            {
                string searchTitle;
